...
export interface ValidatorFn {
  (control: AbstractControl<any>): Promise<ValidationErrors | null> | ValidationErrors | null;
}
...
export abstract class AbstractControl<T = any> {
   ...
   *
   * @param opts The function or array of functions that is used to determine the validity of
   *     this control synchronously.
   */
  constructor(opts: AbstractControlOptions) {
    this.setValidators(opts.validators ? opts.validators : null);
  }
  ...
  get validator(): ValidatorFn | null {
    return (control: AbstractControl) => {
      const errors = mergeErrors(this._validators?.map(validator => validator(control)));
      return errors === null || Object.keys(errors).length === 0 ? null : errors;
    };
  }
  set validator(validatorFn: ValidatorFn | null) {
    this._validators = [validatorFn];
  }
   ...
   */
  abstract setValue(value: any, options?: ObjectType): void;

  /**
   * Patches the value of the control. Abstract method (implemented in sub-classes).
   */
  abstract patchValue(value: any, options?: ObjectType): void;

  /**
   * Resets the control. Abstract method (implemented in sub-classes).
   */
  abstract reset(value?: any, options?: ObjectType): void;
  ...
  /** @internal */
  abstract _forEachChild(cb: FunctionType): void;

  /** @internal */
  abstract _anyControls(condition: FunctionType): boolean;
  ...
  /** @internal */
  _onDisabledChange: FunctionType[] = [];
...
/*
export abstract class AbstractControl1<T = any> {

  protected constructor(opts: AbstractControlOptions) {
    //TODO other options
...
export class FormControl<T> extends AbstractControl<T> {
   ...
   *
   */
  constructor(value: T, opts: AbstractControlOptions = {}) {
    super(opts);

    this._applyFormState(value);
    this._setUpdateStrategy(opts);
  ...
  // @ts-ignore
  _anyControls(condition: FunctionType): boolean {
    return false;
  }
   ...
   * @internal
   */
  _unregisterOnChange(fn: FunctionType): void {
  ...
  // @ts-ignore
  // eslint-disable-next-line @typescript-eslint/no-empty-function
  _forEachChild(cb: FunctionType): void {}
...
export const control = <T>(value: T, opts?: AbstractControlOptions) => new FormControl<T>(value, opts);

type TypeWithGeneric<T> = T[];
type extractGeneric<Type> = Type extends TypeWithGeneric<infer X> ? X : never;
type ArrayControl<T> = T extends Record<string, any> ? FormGroup<T> : FormControl<T>;
...
export class FormArray<T> extends AbstractControl<T> {
   ...
   */
  //TODO: any ??
  constructor(public controls: ArrayControl<extractGeneric<T>>[] | any, opts: AbstractControlOptions = {}) {
    super(opts);
    this._initObservables();
    this._setUpdateStrategy(opts);
    this._setUpControls();
...
export const array = <T>(controls: ArrayControl<extractGeneric<T>>[] | any, opts?: AbstractControlOptions) =>
  new FormArray<T>(controls, opts);

type GroupControls<T> = {
  [Property in keyof T]: T[Property] extends Array<any>
    ? FormArray<T[Property]>
    : T[Property] extends Record<string, any>
    ? FormGroup<T[Property]>
...
export class FormGroup<T> extends AbstractControl<T> {
   ...
   *
   */
  constructor(public controls: GroupControls<T>, opts: AbstractControlOptions = {}) {
    super(opts);
    this._initObservables();
    this._setUpdateStrategy(opts);
    this._setUpControls();
  ...
  /** @internal */
  _reduceChildren(initValue: any, fn: FunctionType) {
    let res = initValue;
    this._forEachChild((control: AbstractControl, name: string) => {
...
export const group = <T>(controls: GroupControls<T>, opts?: AbstractControlOptions) => new FormGroup<T>(controls, opts);
